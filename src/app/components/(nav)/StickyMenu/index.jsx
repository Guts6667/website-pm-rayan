    ...
    <>
      <header className="fixed z-[80] flex gap-[6px] items-center px-[24px] lg:px-[48px] py-[24px]  top-0 self-center w-full   ">
        ...
        >
          ...
          {language === "en" ? (
            <span className="text-sm">{!isActive ? "Menu" : "Close"}</span>
          ) : (
            <span className="text-sm">{!isActive  ? "Menu" : "Fermer"}</span>
          )}
        </div>
        <div className="flex flex-1 gap-[12px] items-center">
          ...
          <Link
            href="/contact"
            className="max-lg:hidden hover:text-orange dark:hover:text-yellow text-2xl transition-all duration-300"
          >
            CONTACT
          </Link>
        </div>
