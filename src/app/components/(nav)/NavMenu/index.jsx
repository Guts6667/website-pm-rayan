...
export default function NavMenu({ language, isActive }) {
  return (
    ...
    >
      <ul >
        <li className={`${isActive ? "transition delay-500 duration-1000 transition-opacity-0" : ""}`}>
          <Link href="/">{language == "en" ? "HOME" : "ACCUEIL"}</Link>
        </li>
        <li className={`${isActive ? "transition delay-500 animate-fadeIn" : ""}`}>
          <Link href="/about">{language == "en" ? "ABOUT" : "A PROPOS"} </Link>
        </li>
        <li className={`${isActive ? "transition delay-700 animate-fadeIn" : ""}`}>
          <Link href="/work">{language == "en" ? "PROJECTS" : "PROJETS"}</Link>
        </li>
        <li className={`${isActive ? "transition delay-1000 animate-fadeIn" : ""}`}>
